...
BATCH_ID = "fandom-priority-2-batch-04-owner-decisions-v1"


def _load_module():
    path = ROOT / "scripts" / "apply_fandom_priority_two_decisions.py"
    spec = importlib.util.spec_from_file_location(
        "fandom_priority_two_decisions_batch_four", path
    )
...
class FandomPriorityTwoDecisionBatchFourTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.result_path = DIRECTORY / "priority-2-batch-04-adjudicated-queue.json"
        cls.report_path = DIRECTORY / "priority-2-batch-04-adjudication-result.md"
        cls.manifest_path = (
    ...
    def _build(self, decisions_path: Path | None = None):
        return self.module.apply_priority_two_decisions(
            ...
            DIRECTORY / "priority-2-batch-04-research.json",
            DIRECTORY / "priority-2-batch-04-research-manifest.json",
            decisions_path or DIRECTORY / "priority-2-batch-04-decisions.json",
            result_version="fandom-2025-priority-two-batch-04-adjudicated-v1",
            batch_label="04",
        )

    def test_cumulative_queue_has_44_completed_and_nine_pending(self) -> None:
        summary = self.result["summary"]
        self.assertEqual(summary["completed_decisions"], 44)
        self.assertEqual(summary["pending_decisions"], 9)
        self.assertEqual(summary["accepted_family_merges"], 4)
        self.assertEqual(summary["accepted_new_casting_families"], 36)
        self.assertEqual(summary["held_family_decisions"], 4)
        self.assertEqual(summary["held_release_variants"], 87)
        self.assertEqual(summary["promotion_eligible_families"], 0)

    def test_batch_accepts_exactly_eight_creations_and_two_holds(self) -> None:
        current = [
            family
        ...
        self.assertEqual(
            sum(value == "create_new_casting" for value in decisions.values()), 8
        )
        self.assertEqual(
            {name for name, value in decisions.items() if value == "hold"},
            {"Mazda MX-5 Miata", "Nissan Skyline 2000GT-R LBWK"},
        )
        self.assertEqual(sum(family["source_row_count"] for family in current), 23)
    ...
    def test_new_decisions_are_attributable_family_only_and_variant_held(self) -> None:
        for family in self.result["families"]:
            ...
            self.assertIn(
                "priority-2-batch-04-research.json#" + family["family_review_id"],
    ...
    def test_all_prior_decisions_and_batch_history_are_preserved(self) -> None:
        ...
        self.assertEqual(len(self.result["decision_batches"]), 5)
    ...
    def test_changed_incomplete_duplicate_and_widened_batches_fail_closed(self) -> None:
        source = DIRECTORY / "priority-2-batch-04-decisions.json"
        ...
        payload = json.loads(source.read_text(encoding="utf-8"))
        payload["batch_id"] = "fandom-priority-2-batch-03-owner-decisions-v1"
        with tempfile.TemporaryDirectory() as directory:
            changed = Path(directory) / "decisions.json"
            changed.write_text(json.dumps(payload), encoding="utf-8")
    ...
    def test_outputs_are_frozen_and_deterministically_reproducible(self) -> None:
        ...
        expected = self.module.expected_outputs(
            *self._build(),
            result_file_name="priority-2-batch-04-adjudicated-queue.json",
            report_file_name="priority-2-batch-04-adjudication-result.md",
        )
        actual = (
