...
def _load_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "fandom_priority_two_research_batch_two", path
    )
...
class FandomPriorityTwoResearchBatchTwoTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.research_path = DIRECTORY / "priority-2-batch-02-research.json"
        cls.report_path = DIRECTORY / "priority-2-batch-02-research.md"
        cls.manifest_path = DIRECTORY / "priority-2-batch-02-research-manifest.json"
    ...
    def _build(self):
        return self.module.build_research(
            ...
            DIRECTORY / "priority-2-batch-02-source-notes.json",
            batch_version="fandom-2025-priority-two-research-batch-02-v1",
            batch_label="02",
        )

    def test_batch_is_next_ten_pending_families_after_batch_one_decisions(self) -> None:
        queue = json.loads(
            (DIRECTORY / "priority-2-batch-01-adjudicated-queue.json").read_text()
        )
        expected = [
        ...
        self.assertNotIn("fandom-family-08cb7fadb9fed12d", actual)

    def test_nine_creates_and_one_homonymous_hold_are_proposed(self) -> None:
        ...
        self.assertEqual(
            [packet["casting_name"] for packet in held],
    ...
    def test_homonymous_hold_records_the_distinct_historical_tool(self) -> None:
        held = next(
            packet
            for packet in self.research["packets"]
            if packet["casting_name"] == "Batman and Robin Batmobile"
        )
        related = held["research_evidence"]["wiki_casting_page"][
            "related_casting_pages"
        ]
        self.assertEqual(len(related), 1)
        self.assertIn("2004", related[0]["distinction"])
        self.assertIn("G5513", related[0]["distinction"])
    ...
    def test_outputs_are_frozen_and_deterministically_reproducible(self) -> None:
        ...
        expected = self.module.expected_outputs(
            *self._build(),
            research_file_name="priority-2-batch-02-research.json",
            report_file_name="priority-2-batch-02-research.md",
        )
        actual = (
