...
BATCH_ID = "fandom-priority-2-batch-03-owner-decisions-v1"


def _load_module():
    path = ROOT / "scripts" / "apply_fandom_priority_two_decisions.py"
    spec = importlib.util.spec_from_file_location(
        "fandom_priority_two_decisions_batch_three", path
    )
...
class FandomPriorityTwoDecisionBatchThreeTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.result_path = DIRECTORY / "priority-2-batch-03-adjudicated-queue.json"
        cls.report_path = DIRECTORY / "priority-2-batch-03-adjudication-result.md"
        cls.manifest_path = (
    ...
    def _build(self, decisions_path: Path | None = None):
        return self.module.apply_priority_two_decisions(
            ...
            DIRECTORY / "priority-2-batch-03-research.json",
            DIRECTORY / "priority-2-batch-03-research-manifest.json",
            decisions_path or DIRECTORY / "priority-2-batch-03-decisions.json",
            result_version="fandom-2025-priority-two-batch-03-adjudicated-v1",
            batch_label="03",
        )

    def test_cumulative_queue_has_thirty_four_completed_and_19_pending(self) -> None:
        summary = self.result["summary"]
        self.assertEqual(summary["completed_decisions"], 34)
        self.assertEqual(summary["pending_decisions"], 19)
        self.assertEqual(summary["accepted_family_merges"], 4)
        self.assertEqual(summary["accepted_new_casting_families"], 28)
        self.assertEqual(summary["held_family_decisions"], 2)
        self.assertEqual(summary["held_release_variants"], 64)
        self.assertEqual(summary["promotion_eligible_families"], 0)

    def test_batch_accepts_all_ten_creation_recommendations(self) -> None:
        current = [
            family
        ...
        self.assertEqual(
            {
                family["reviewer_decision"]["decision"]
                for family in current
            },
            {"create_new_casting"},
        )
        self.assertEqual(sum(family["source_row_count"] for family in current), 18)
    ...
    def test_new_decisions_are_attributable_family_only_and_variant_held(self) -> None:
        for family in self.result["families"]:
            ...
            self.assertIn(
                "priority-2-batch-03-research.json#" + family["family_review_id"],
    ...
    def test_all_prior_decisions_and_batch_history_are_preserved(self) -> None:
        ...
        self.assertEqual(len(self.result["decision_batches"]), 4)
        self.assertEqual(self.result["decision_batches"][-1]["batch_id"], BATCH_ID)

    def test_changed_incomplete_and_duplicate_batches_fail_closed(self) -> None:
        source = DIRECTORY / "priority-2-batch-03-decisions.json"
        ...
        payload = json.loads(source.read_text(encoding="utf-8"))
        payload["batch_id"] = "fandom-priority-2-batch-02-owner-decisions-v1"
        with tempfile.TemporaryDirectory() as directory:
            changed = Path(directory) / "decisions.json"
            changed.write_text(json.dumps(payload), encoding="utf-8")
    ...
    def test_outputs_are_frozen_and_deterministically_reproducible(self) -> None:
        ...
        expected = self.module.expected_outputs(
            *self._build(),
            result_file_name="priority-2-batch-03-adjudicated-queue.json",
            report_file_name="priority-2-batch-03-adjudication-result.md",
        )
        actual = (
