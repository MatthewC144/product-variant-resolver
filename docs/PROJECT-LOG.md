...
## 2026-09-16 — SEL-CRAWL1：完成非Fandom同結構網站的單頁Selenium→XLSX工具

### 新執行了什麼，以及解決什麼問題

Owner說明真正目標不是Fandom，而是另一個HTML結構與Fandom相同的網站，並要求用Selenium取得資料、
最後儲存成XLSX。此時尚未提供真實URL與允許的hostname，因此本輪沒有猜測網站、也沒有發出遠端請求；
完成的是可重用的一頁式collector、動態本機測試頁及三筆synthetic sample workbook。它解決的是「網站會
用JavaScript稍後才放入table，普通靜態request未必看到資料」以及「爬到的列要能由不熟code的人在Excel
檢查來源、顏色、variant與解析錯誤」兩個問題。

實際本機Chrome測試先載入只有空容器的HTML，100ms後才插入`sortable wikitable`。Selenium等待table
出現後讀到四列；年份規則保留三筆2025資料、排除一筆2024資料，並把`Example Roadster (2nd Color -
Zamac)`拆成casting與variant note，同時保留來源直接給的Zamac color。這證明測試涵蓋動態DOM而不是只
測一段手寫JSON；它仍完全不代表未知網站已相容或已允許存取。

### 代碼修改了哪一部分、原因與設計決定

新增`selenium_catalog.py`，把工作分成三層。第一層驗證HTTP(S)、URL沒有內嵌帳密、requested/final host
都精確等於allowlist，並永久拒絕Fandom/Wikia；選擇精確host而不是寬鬆suffix，是避免允許
`catalog.example.com`後被redirect到另一個subdomain或第三方。第二層用Selenium Chrome等待Fandom-like
table並擷取text DOM，停用圖片與下載；若頁面出現CAPTCHA、access denied或rate limit文字就停止，沒有
登入、proxy rotation或解題分支。第三層是純Python header mapper與year filter；選擇看欄名而不是固定第
幾欄，是因為同結構網站仍可能調換Toy#、Model、Series、Color的位置。

新增`crawl_fandom_like_catalog.py`作為新手可執行CLI。`--confirm-permission`不是法律證明，但能避免只貼
URL就無意啟動；`--allowed-host`把執行範圍寫得可見；`--max-records`預設3,000並硬上限5,000，是對齊未來
資料規模又避免無界記憶體輸出。第一版只讀一頁，不做pagination或link discovery；因為真實網站尚未知，
現在設計crawl frontier會是在沒有rules/path證據時過度擴張。

XLSX runtime先完成`Summary`和`Releases`兩張sheet：Summary用公式計算實際輸出列與錯誤列；Releases保留
normalized欄、source page/table/row、UTC、raw JSON與parse error，並提供filter和freeze pane。開發時原先
讓CLI依賴artifact-tool Node exporter，但隨後實際查詢npm registry得到404，證明這是Codex bundled套件，
不能讓GitHub使用者公開安裝。於是runtime改成公開可安裝的OpenPyXL，維持Python-only使用流程；
artifact-tool builder只保留做sample workbook的formula inspection與render QA，不再是collector執行依賴。
這次重新選型犧牲單一writer的一致性，但避免交付只在本機能跑的repo。

`pyproject.toml`把Selenium放在`crawler`optional extra，而不是主dependencies，因為FastAPI/Dual RAG
runtime不需要Chrome。新增本機HTML、正規化unit tests、opt-in browser smoke test與直接解析XLSX OpenXML
的artifact test。後者確認Summary/Releases、公式、table與`001`以string cell保存；另一項unit test也把
OpenPyXL輸出重新讀回，確認公式、文字識別碼與freeze pane。這避免Excel把collector
number前導零吃掉。第一次render也確實發現UTC顯示成Excel serial，修正date-time format後再輸出與檢查，
證明視覺QA有影響實作而不是形式動作。

### 驗證、失敗紀錄與目前限制

聚焦測試6項全部PASS，其中browser test真的啟動本機ChromeDriver，但需要sandbox允許loopback port。
完整測試第一次直接執行`.venv/bin/pytest`時有1項既有subprocess測試因找不到editable package失敗；這不是
新增演算法失敗，而是該測試需要repo慣用的`PYTHONPATH=src`。用正確環境重跑後結果為615 passed、1個
預設停用browser test skipped、1個既有Starlette/AnyIO deprecation warning。新增檔案的Ruff、strict MyPy、
compileall、Node syntax與fixture JSON均PASS；全repo Ruff/MyPy仍有大量既有且與本功能無關的baseline debt，
本輪沒有為了製造綠色數字而改動歷史程式。

XLSX recalculation後Summary顯示3 records、0 parse errors；formula error scan為0，兩個sheet都完成render
檢查。sample只含`catalog.example.test`synthetic資料，不是遠端網站資料，也沒有寫PostgreSQL、canonical
catalog、Dual RAG corpus/evaluation或FastAPI。下一步必須由owner提供真實非Fandom URL、exact host、允許
自動讀取的依據及最好一份saved HTML；先檢查欄名與規則，再執行單頁canary。Pagination、3,000筆效能與
DB ingestion只能在這個canary人工review後另立spec，不能從本次完成狀態自動推進。
